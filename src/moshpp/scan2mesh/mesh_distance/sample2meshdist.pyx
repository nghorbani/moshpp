...
cdef somedistance(SomeDistance distance_to,
    ...
    if compute_dsample:
        if dsample_indices is not None and dsample_indptr is not None and dsample_sortdata is not None:
            ...
            Dr_samplev = scipy.sparse.csc_matrix((dsample_v[dsample_sortdata], dsample_indices, dsample_indptr),
                                                 (S, 3 * S))
        else:
            js = n.arange(3 * S)
...
cdef somedistance_scalar(SomeDistance distance_to,
    ...
    for ss in range(S):
        ...
        f += distance_to.tri(nearest_part[ss], &sample_v[ss, 0],
                             &ref_v[ref_tri[tt, 0], 0], &ref_v[ref_tri[tt, 1], 0], &ref_v[ref_tri[tt, 2], 0],
                             &dsample[3 * ss], &dref[3 * ref_tri[tt, 0]], &dref[3 * ref_tri[tt, 1]],
                             &dref[3 * ref_tri[tt, 2]])
    return f, dref, dsample

## type-specializations to make this stuff available in Python
## geman-mcclure stuff untested
def distance(nearest_tri, nearest_part, ref_tri, ref_v, sample_v, compute_dref=True, compute_dsample=True,
             dsample_pattern={}):
    cdef Distance f
    return somedistance[Distance](f, nearest_tri, nearest_part, ref_tri, ref_v, sample_v, compute_dref, compute_dsample,
                                  dsample_pattern.get('indices'), dsample_pattern.get('indptr'),
                                  dsample_pattern.get('sortdata'))

def squared_distance(nearest_tri, nearest_part, ref_tri, ref_v, sample_v, compute_dref=True, compute_dsample=True,
                     dsample_pattern={}):
    cdef SquaredDistance f
    return somedistance[SquaredDistance](f, nearest_tri, nearest_part, ref_tri, ref_v, sample_v, compute_dref,
                                         compute_dsample,
                                         dsample_pattern.get('indices'), dsample_pattern.get('indptr'),
                                         dsample_pattern.get('sortdata'))

def gm_distance(sigma, nearest_tri, nearest_part, ref_tri, ref_v, sample_v, compute_dref=True, compute_dsample=True,
                dsample_pattern={}):
    cdef GMDistance * f = new GMDistance(sigma)
    return somedistance[GMDistance](cython.operator.dereference(f), nearest_tri, nearest_part, ref_tri, ref_v, sample_v,
                                    compute_dref, compute_dsample,
                                    dsample_pattern.get('indices'), dsample_pattern.get('indptr'),
                                    dsample_pattern.get('sortdata'))
...
def gm_distance_scalar(sigma, nearest_tri, nearest_part, ref_tri, ref_v, sample_v):
    cdef GMDistance * f = new GMDistance(sigma)
    return somedistance_scalar[GMDistance](cython.operator.dereference(f), nearest_tri, nearest_part, ref_tri, ref_v,
                                           sample_v)
